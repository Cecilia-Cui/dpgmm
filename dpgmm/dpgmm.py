...
class DPGMM:
  ...
  def __init__(self, dims, stickCap = 1):
    ...
    if isinstance(dims, DPGMM):
      ...
      self.n = [gcp.GaussianPrior(x) for x in dims.n]
      ...
      self.nT = [None if (x is None) else gcp.StudentT(x) for x in dims.nT]
      ...
      self.priorT = None
      self.n = [gcp.GaussianPrior(self.dims) for _ in range(self.stickCap)] # The mixture components, one for each stick.
  ...
  def incStickCap(self, inc = 1):
    ...
    self.stickCap += inc
    self.n += [gcp.GaussianPrior(self.dims) for _ in range(inc)]
  ...
  def sampleMixture(self):
    ...
    gauss = [x.sample() for x in self.n]
